...
def main():
    ...
    #5 / texto = "while bitcoin itself is intangible in order for us to engage with the network we have to interact with points of contact along the way"
    #5 / texto = "private keys effectively the password to your account generated by cryptographic hashing algorithms are incredibly challenging if not impossible to memorize and their format speaks to the nature of a private key"
    
    #3 + texto = "backup seed phrases are mnemonic devices used to backup and restore access to wallets"
    #8 / texto = "BIP-39 seed phrases were implemented to create an english language backup to be kept entirely offline"
    ...
    #16 / texto = "carrying a list of seed phrases along your journey could be considered suspect behavior and put any individual at risk even the average person who stores their seed phrases in a single physical location could have their funds stolen if they are found by someone familiar with the process of restoring from backup phrases"

    #6 + texto = "how can we design for these fringe cases today to build individual freedom and agency for those living under unstable dictatorships persecution or war"
    #9 + texto = "how can we help people safely travel with and store their value in times where fiat is failing gold is in high demand and at risk of theft"
    #6 + texto = "how can we build systems for the everyday person to be able to conceal their seed phrases in ways to avoid detection extortion and confiscation"
    #15 - texto = "how can we design for these fringe cases today to build individual freedom and agency for those living under unstable dictatorships persecution or war how can we help people safely travel with and store their value in times where fiat is failing gold is in high demand and at risk of theft"
    #15 - texto = "how can we help people safely travel with and store their value in times where fiat is failing gold is in high demand and at risk of theft how can we build systems for the everyday person to be able to conceal their seed phrases in ways to avoid detection extortion and confiscation"

    #5 / texto = "how can we best enable the safe migration of people and their value"

    #9 + texto = "cryptography means secret writing in greek the technique dating back nearly four thousand years to ancient egypt defined as the art of protecting information by transforming it into an unreadable format called cipher text cryptography is a critical element in the suite of technologies that make bitcoin possible"
    #3 + texto = "the art of protecting information by transforming it into an unreadable format called cipher text"
